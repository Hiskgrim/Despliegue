...
function getTabla(idProyecto, datos, columnas) {
    return {
      style: 'tabla',
        table: {
            headerRows: 1,
            body: getCuerpoTabla(idProyecto, datos, columnas)
        }
    };
}

function getPreambuloTexto(preambulo){
	return {
		text: preambulo,
		style: 'texto'
	};
...
function getConsideracionTexto(consideracion){
	return {
		text: consideracion,
		style: 'texto'
	};
...
function getArticuloTexto(articulo, numero){
	var aux='ARTÍCULO '+numeroALetras(numero+1)+' - '+articulo.texto;
	if(articulo.paragrafo!=null){
		aux=aux+'\n\nPARAGRAFO - '+articulo.paragrafo;
	}
	return {
		text: aux,
		style: 'texto'
...
getContenido=function(elemento){
  	var contenido = [];
  	contenido.push({ text: 'RESOLUCIÓN No '+elemento.numero,
	    style: 'titulo'});
  	contenido.push(getPreambuloTexto(elemento.preambulo));
  	contenido.push({ text: 'CONSIDERANDO',
        style: 'titulo'});
  	contenido.push(getConsideracionTexto(elemento.consideracion));
  	contenido.push({ text: 'RESUELVE',
        style: 'titulo'});
  	var numero=0;
  	elemento.articulos.forEach(function(articulo){
  		contenido.push(getArticuloTexto(articulo, numero));
  		if(articulo.asociado){
  			elemento.proyectos.forEach(function(proyecto){
  				contenido.push({ text: proyecto.Nombre,
        			style: 'proyecto'});
  	  			contenido.push(getTabla(proyecto.Id, elemento.contratados, ['Nombre', 'Documento', 'Expedicion', 'Categoria', 'Dedicacion', 'HorasSemanales', 'PeriodoVinculacion', 'ValorContrato']));
  			});
  		}
  		numero++;
  	})
  	contenido.push({ text: 'COMUNIQUESE Y CUMPLASE',
		style: 'finalizacion'});
  	contenido.push({ text: '-- NOMBRE DEL DECANO --',
		style: 'nombre'});
...
getDocumento=function(elemento){
  var documento={
    info: {
      title: 'Resolución'
    },
...
getPreambulo=function(){
  return preambulo;
};

getConsideracion=function(){
